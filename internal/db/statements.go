...
func initStmt(db *sql.DB) {
	...
	stmtAddSession, err = db.PrepareContext(context.Background(),
		`INSERT INTO sessions (id, userId) VALUES($1,$2)`)
	if err != nil {
		log.Println(err.Error())
		return
	}
	stmtDeleteSession, err = db.PrepareContext(context.Background(),
		`DELETE FROM sessions WHERE id=$1`)
	if err != nil {
		log.Fatal(err.Error())
		return
	}
	stmtGetSession, err = db.PrepareContext(context.Background(),
